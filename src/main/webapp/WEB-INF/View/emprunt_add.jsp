	          ...
	          <div class="input-field col s6">
	            <select id="idLivre" name="idLivre" class="browser-default">
	              ...
	              <c:forEach items="${ livresDispo }" var="livre">
                  	<option value="<c:out value="${ livre.getId() }" />">"<c:out value="${ livre.getTitre() }" />", de <c:out value="${ livre.getAuteur() }" /></option>
                  </c:forEach>
	            </select>
	          </div>
	          <div class="input-field col s6">
	            <select id="idMembre" name="idMembre" class="browser-default">
	              ...
	              <c:forEach items="${ membresPossibles }" var="membre">
                  	<option value="<c:out value="${ membre.getId() }" />"><c:out value="${ membre.getPrenom() }" /> <c:out value="${ membre.getNom() }" /></option>
                  </c:forEach>
	            </select>
	          </div>
	        </div>
