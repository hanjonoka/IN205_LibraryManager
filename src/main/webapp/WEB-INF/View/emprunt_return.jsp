	            ...
	            <select id="id" name="id" class="browser-default">
                  ...
                  <!-- TODO : si l'attribut id existe, l'option correspondante devra être sélectionnée par défaut (ajouter l'attribut selected dans la balise <option>) -->
	                <c:forEach items="${emprunts_current}" var="emprunt">
	                	<option value="<c:out value="${ emprunt.getId() }" />" <c:if test="${ emprunt.getId() ==  id }">selected</c:if> >
	                	    <c:out value="${ emprunt.getLivre().getTitre() }" />, <em>de <c:out value="${ emprunt.getLivre().getAuteur() }" />,</em>
	                		emprunté par <c:out value="${ emprunt.getMembre().getPrenom() }" /> <c:out value="${ emprunt.getMembre().getNom() }" />
	                	</option>
					</c:forEach>
	            </select>
	          </div>
